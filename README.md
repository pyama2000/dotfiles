...
5. git のローカル設定書き込み
6. Nix 管理外のものを個別に導入: `aqua i`、`rustup`、`asdf`（nodejs/python ランタイム）、lima VM（`docker` / `docker-rosetta`）と docker context

Linux では apt 依存パッケージ・Nix の導入後、`home-manager switch --flake ~/dotfiles/nix#takahiko-yamashita@<x86_64|aarch64>-linux` を実行し、同様に git 設定・`aqua`・`rustup`・`asdf` を導入します。
...
- CLI ツール本体・dotfiles symlink・Git 設定 / fish / starship: `nix/home-manager/`
...
3. **Homebrew**（`nix-darwin` の `homebrew` モジュール、`nix/darwin/homebrew.nix`）— `git`（ブートストラップ）、`python@3.9`（EOL で nixpkgs から削除済み）、`readline` / `xz`（asdf の python ビルド依存）、`aqua` バイナリ本体、GUI cask
4. **ネイティブのマネージャ**（残り全部）— `rustup`、`asdf`（Node / Python のバージョン切替用）、nixpkgs に無い一部の cargo/uv/npm/go 製ツール（`kani-verifier` / `awscli-local` / `cronv` / `sshtunnel` / MCP サーバ / `@playwright/cli` など）
...
6. `aqua i`・`rustup update`・`asdf plugin update --all` を実行
7. macOS では `brew update && brew upgrade && brew upgrade --cask && brew autoremove && brew cleanup`
8. Neovim プラグインを更新（`Lazy! update` / `MasonUpdate`）、`nvim/lazy-lock.json` に差分があれば自動でコミット・push
