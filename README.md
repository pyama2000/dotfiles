...
7. `npx --yes skills update --global --yes` で `~/.agents/skills` のエージェント用スキルを更新（リポジトリ外のためコミットなし。失敗しても警告して続行）
8. macOS では `brew update && brew upgrade && brew upgrade --cask && brew autoremove && brew cleanup`
9. Neovim プラグインを更新（`Lazy! update`）、`nvim/lazy-lock.json` に差分があれば自動でコミット・push
10. macOS では `Brewfile` を再生成（`brew bundle dump --force --formulae --casks --taps --go --cargo --uv --npm`。Homebrew 6 で `--all` は廃止）、差分があれば自動でコミット・push
