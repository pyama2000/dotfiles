...

{
  programs.fish = {
    ...
    # zoxide / fzf / direnv / mise / starship の init 注入は
    # 各 programs.* の enableFishIntegration が担うため、ここでは source のみ行います。
    # atuin だけは init を fish/interactive.fish 側で行い（enableFishIntegration=false）、
    # --disable-up-arrow 等のフラグを rebuild なしで編集できるようにしています。
